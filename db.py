...
def get_session(username):
    username = MySQLdb.escape_string(username)
    sql = "SELECT sid,last_visit_time FROM yagra_session WHERE username='%s'" % username
    n = cursor.execute(sql)
    if n == 0:
        return None
    row = cursor.fetchone()
...
def set_session(username, sid, now_time):
    ...
    sql = "UPDATE yagra_session SET sid='%s',last_visit_time='%s' WHERE username='%s'" % (sid, now_time, username)
    n = cursor.execute(sql)
    return n == 1
    
def set_session_time(username, now_time):
    username = MySQLdb.escape_string(username)
    sql = "UPDATE yagra_session SET last_visit_time='%s' WHERE username='%s'" % (now_time, username)
    n = cursor.execute(sql)
    return n == 1
...
def add_session(username, sid):
    ...
    sid = MySQLdb.escape_string(sid)
    now_time = time.time()
    sql = "INSERT INTO yagra_session(username, sid, last_visit_time) VALUES('%s', '%s', '%s')" % (username, sid, now_time)
    n = cursor.execute(sql)
    return n == 1
...
def set_session_sid(username, sid):
    username = MySQLdb.escape_string(username)
    sid = MySQLdb.escape_string(sid)
    sql = "UPDATE yagra_session SET sid='%s' WHERE username='%s'" % (sid, username)
    n = cursor.execute(sql)
    return n == 1
